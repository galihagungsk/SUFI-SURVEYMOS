...
function tampilkanHasil(data) {
  hasilDiv.innerHTML = `
    ...
    <p><strong>Jenis Pembelian:</strong> ${data.jenis}</p>
    <p><strong>Model:</strong> ${data.model}</p>
    <p><strong>Harga OTR:</strong> Rp ${data.harga.toLocaleString("id-ID")}</p>
    <p><strong>DP (${data.dpPersen}%):</strong> Rp ${data.dpNominal.toLocaleString("id-ID")}</p>
    <p><strong>Tenor:</strong> ${data.tenor} bulan</p>
    <p><strong>Bunga:</strong> ${data.bunga}%</p>
    <p><strong>Cicilan per Bulan:</strong> Rp ${data.cicilanBulanan.toLocaleString("id-ID")}</p>
    <hr>
    <p><strong>Nama:</strong> ${data.nama}</p>
    <p><strong>No. Telepon:</strong> ${data.telepon}</p>
    <p><strong>Alamat:</strong> ${data.alamat}</p>
    <p style="color: green; font-style: italic;">✅ Data disimpan ke cache browser</p>
  `;
}